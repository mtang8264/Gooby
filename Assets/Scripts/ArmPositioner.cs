...
public class ArmPositioner : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        float angle = 0f;                  // You can the find the arctan of that to find the angle and I converted that to degrees.
        if (run > float.Epsilon || run < -float.Epsilon)
        {
            angle = Mathf.Atan(rise / run) * Mathf.Rad2Deg;
            angle += 90f;
        }
        Vector3 rot = new Vector3(0, 0, angle);                             // Because Unity has 0 degrees as up I had to rotate an additional 90 degrees.
